...
public class SessaoTest {
	...
	public void testListarSessoesSucesso() {
		 ...
		 * a quantidade de sessões encontradas condiz com a quantidade anteriormente cadastrada e se 
		 * os dados são iguais.
		 */
		
		Iterador iterador = controllerAdministrador.listarSessoes(salaTeste);
		int cont = 0;
		...
		assertEquals(15, ((Sessao) iterador.obterProximo()).getHoraInicio());
	...
	@Test
	public void testAlterarSessaoNaoCadastrada() {
		/* Cria novos cinema, nova sala e nova sessão. */
		Cinema cine = new Cinema();
		...
		Sala sala = new Sala();
		sala.setNumeroSala(1);
		sala.setQtdCadeiras(10);
		
		Sessao sessao = new Sessao();
		sessao.setHoraInicio(10);
		sessao.setHoraFim(12);
		
		/* Realiza o cadastro do cinema. */
		try{
		...
		/* Certifica-se que ao tentar realizar a alteração do cadastro de uma sessão não cadastrada a exceção SessaoNaoEncontradaException
		 * é lançada.
		 */		
		try {
			controllerAdministrador.alterarSessao(cine, sala, 10, 12, 15, 17);
		} catch (SessaoNaoEncontradaException e) {
			assertTrue(true);
		} catch (CinemaNaoEncontradoException e) {
			fail();
